...
data[7].to_csv('/data/process/olist_geolocation_dataset_clean.csv', index=False) #Simpan sebagai csv

#Bersihkan data cutomer
data[3]['customer_city'] = data[3]['customer_city'].apply(normalize)
data[3].to_csv('/data/process/olist_customer_dataset_clean.csv', index=False)

#Bersihkan data seller dan isi yang kosong dan aneh
data[4]['seller_city'] = data[4]['seller_city'].apply(normalize)
data[4].to_csv('/data/process/olist_sellers_dataset_clean.csv', index=False)

#Bersihkan aksen di produk
data[6]['product_category_name'] = data[6]['product_category_name'].apply(normalize)
data[6].to_csv('/data/process/olist_product_dataset_clean.csv', index=False)

#Bersihkan nama di kategori
data[2]['product_category_name'] = data[2]['product_category_name'].apply(normalize)
data[2].to_csv('/data/process/olist_product_category_dataset_clean.csv', index=False)
