...
loc_fix = pd.read_csv('/data/process/olist_geolocation_dataset_clean.csv')
...
def fix_loc(path, kota, zip_code, nama_file):
    ...
    data.to_csv(f'/data/final/{nama_file}.csv', index=False)

#fix data seller
seller_path = "/data/process/olist_sellers_dataset_clean.csv"
fix_loc(seller_path, "seller_city", "seller_zip_code_prefix", "olist_sellers_dataset_final")

#fix data customer
cust_path = "/data/process/olist_customer_dataset_clean.csv"
...
product_data = pd.read_csv("/data/process/olist_product_dataset_clean.csv")
product_english_data = pd.read_csv("/data/process/olist_product_category_dataset_clean.csv")
...
product_data.to_csv('/data/final/olist_product_dataset_final.csv', index=False)
...
order_data.to_csv('/data/final/olist_orders_dataset_final.csv', index=False)


#frature engineering order items data
ordr_item_data = pd.read_csv("/data/raw/olist_order_items_dataset.csv")
...
ordr_item_data.to_csv('/data/final/olist_order_items_dataset_final.csv', index=False)

# add id to payment
payment = pd.read_csv("/data/raw/olist_order_payments_dataset.csv")
payment["payment_id"] = [uuid4().hex for _ in range(len(payment))]
payment.to_csv('/data/final/olist_order_payments_dataset_final.csv', index=False)